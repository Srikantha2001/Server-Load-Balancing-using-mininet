...
class iplb (object):
  ...
  def __init__ (self, connection, service_ip, servers = []):
    ...
    self.freq ={} #freq of request to each server
  ...
  def _pick_server (self, key, inport):
    ...
    """
    allEqual = True
    testVal = self.freq[random.choice(list(self.live_servers.keys()))]

    for ele in self.freq:
      if self.freq[ele] != testVal:
        allEqual = False
        break

    if(allEqual):
      server_ip = random.choice(list(self.live_servers.keys()))
      self.freq[server_ip] += 1
      print(self.freq)
      # print("Selected server :",server_ip)
      return server_ip
    else:
      server_ip = min(self.freq,key=self.freq.get)
      self.freq[server_ip] += 1
      # print("Selected server :", server_ip)
      print(self.freq)
      return server_ip
